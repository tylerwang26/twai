...
const user = process.argv[3] || "U03d92f2cc0d998fcf4c81e69735e12ee";

async function runAll() {
    const watchlistPath = "documents/mir_logic/watchlist.md";
    if (!fs.existsSync(watchlistPath)) {
        console.error("Watchlist file not found.");
        return;
    }
    const content = fs.readFileSync(watchlistPath, 'utf8');
    const tickerRegex = /## \d+\. .*?\(([\d\w.]+)\)/g;
    let match;
    const tickers = [];
    while ((match = tickerRegex.exec(content)) !== null) {
        if (!tickers.includes(match[1])) tickers.push(match[1]);
    }
    console.log(`Watchlist tickers found: ${tickers.join(', ')}`);
    for (const t of tickers) {
        await renderUltimateChart(t, generateData(350), user);
